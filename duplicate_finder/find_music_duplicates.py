...
def main():
    ...
    if cats["duplicate"]:
        if CONFIRM_CSV:
            ...
            # Metadata duplicates — Notepad list or no-review mode
            if meta_dups:
                if NO_REVIEW:
                    # Mark all as pending — user will review in the browser viewer
                    for sub_idx in range(len(meta_dups)):
                        dup_selected.add(meta_dup_indices[sub_idx])
                    print(f"\n  Standard duplicates ({len(meta_dups)}): marked as pending for viewer review.")
                else:
                    content = build_notepad_list(
            ...
            # Fingerprint duplicates — tiered Notepad lists or no-review mode
            if fp_dup_indices:
                if NO_REVIEW:
                    dup_selected.update(fp_dup_indices)
                    print(f"\n  Fingerprint duplicates ({len(fp_dup_indices)}): marked as pending for viewer review.")
                else:
    ...
    if cats["better"]:
        if CONFIRM_CSV:
            ...
            # Metadata better quality — single list or no-review mode
            if meta_better:
                if NO_REVIEW:
                    for sub_idx in range(len(meta_better)):
                        better_selected.add(meta_better_indices[sub_idx])
                    print(f"\n  Higher quality matches ({len(meta_better)}): marked as pending for viewer review.")
                else:
            ...
            # Fingerprint better quality — tiered lists or no-review mode
            if fp_better_indices:
                if NO_REVIEW:
                    better_selected.update(fp_better_indices)
                    print(f"\n  Fingerprint higher quality ({len(fp_better_indices)}): marked as pending for viewer review.")
                else:
