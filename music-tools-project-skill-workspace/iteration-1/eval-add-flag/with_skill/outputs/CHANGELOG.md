...
### Added
- `--depth N` flag: limits the scan to folders that are exactly N levels below the root. For example, `--depth 2` only inspects folders at `root/level1/level2` and ignores shallower or deeper folders. When omitted, the script scans all depths as before.
