...
def find_cd_folders(root: Path, depth_limit: int | None = None) -> list[dict]:
    """Find all folders matching 'CD #' pattern under root.

    If depth_limit is given, only folders exactly that many levels below root
    are considered. For example, depth_limit=2 matches folders at
    root/level1/level2 but not root/level1 or root/level1/level2/level3.
    """
    candidates = []
    for folder in sorted(root.rglob("*")):
        ...
        # Depth filtering: count how many parts separate folder from root
...
if __name__ == "__main__":
    ...
    depth_display = str(DEPTH_LIMIT) if DEPTH_LIMIT is not None else "all (no limit)"
    ...
    print(f"  Mode   : {mode}")
    print(f"  Folder : {root}")
    print(f"  Depth  : {depth_display}")
    print(f"{'=' * 60}\n")

    candidates = find_cd_folders(root, depth_limit=DEPTH_LIMIT)
