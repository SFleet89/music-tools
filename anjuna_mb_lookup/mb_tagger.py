...
MusicBrainz, writes tags to files, embeds cover art, and renames folders.

Use this with the output of mb_lookup.py (or any lookup CSV exported from
the viewer with confirmed MBIDs).

Requirements:
    pip install mutagen

Usage:
    python mb_tagger.py                           # opens file picker, dry run
    python mb_tagger.py --apply                   # opens file picker, tag for real
    python mb_tagger.py "path\\to\\lookup.csv"    # dry run with specific CSV
    python mb_tagger.py "path\\to\\lookup.csv" --apply
    python mb_tagger.py "path\\to\\lookup.csv" --apply --skip-art

Output:
    Reports saved to: <script folder>\\reports\\mb_tagger_YYYYMMDD_HHMMSS.csv
"""
...
def get_album_artist(release):
    ...
    for c in credits:
        if isinstance(c, dict):
            ...
            if name: parts.append(name + join)
...
def get_tracks(release):
    ...
    for disc_idx, medium in enumerate(media, 1):
        ...
        for t in medium_tracks:
            ...
            t_credits = (t.get("artist-credit") or rec.get("artist-credit") or
            ...
            for c in t_credits:
                if isinstance(c, dict):
                    name = c.get("name") or (c.get("artist") or {}).get("name", "")
                    join = c.get("joinphrase", "")
                    if name: t_parts.append(name + join)
            tracks.append({
...
def build_folder_name(release, album_artist, year):
    ...
    if year: return "%s - %s (%s)" % (artist, title, year)
...
def get_music_files(folder):
    ...
    if cd_dirs and len(cd_dirs) == len(subdirs):
        ...
        for cd in sorted(cd_dirs):
            files.extend(sorted(f for f in cd.iterdir()
...
def write_tags_flac(path, track, release, album_artist, year, label, catno, art_bytes):
    ...
    if art_bytes:
        pic = Picture(); pic.type=3; pic.mime="image/jpeg"; pic.desc="Cover"; pic.data=art_bytes
...
def write_tags(path, track, release, album_artist, year, label, catno, art_bytes):
    ...
    if ext == ".flac":          write_tags_flac(path, track, release, album_artist, year, label, catno, art_bytes)
    elif ext == ".mp3":         write_tags_mp3(path, track, release, album_artist, year, label, catno, art_bytes)
    elif ext in (".m4a",".aac"):write_tags_m4a(path, track, release, album_artist, year, label, catno, art_bytes)
    else:
        audio = MutagenFile(path, easy=True)
        if audio:
            audio["title"]=[track["title"]]; audio["artist"]=[track["artist"] or album_artist]
            audio["albumartist"]=[album_artist]; audio["album"]=[release.get("title","")]
            audio["tracknumber"]=["%d/%d"%(track["track_number"],track["total_tracks"])]
            audio["discnumber"]=["%d/%d"%(track["disc_number"],track["disc_count"])]
            audio["date"]=[year]; audio.save()
...
def filter_actionable(rows):
    return [r for r in rows if r.get("mbid","").strip() and r.get("folder_path","").strip()]
...
def process_folder(row, dry_run, skip_art):
    ...
    result = {"folder": folder_name, "folder_path": str(folder_path), "mbid": mbid,
              "status": "", "new_folder": "", "files_tagged": 0, "notes": ""}

    if not folder_path.exists():
        result.update(status="error", notes="Folder not found")
        return result

    files = get_music_files(folder_path)
    if not files:
        result.update(status="skipped", notes="No music files found")
        return result

    try:
        release = mb_fetch_release(mbid)
    except Exception as e:
        result.update(status="error", notes="MB fetch failed: %s" % e)
        return result
    ...
    if dry_run:
        result.update(status="would_tag", files_tagged=len(pairs),
                      notes=count_note or "would tag %d file(s)" % len(pairs))
        return result
    ...
    if folder_path != new_folder_path:
        try:
            if new_folder_path.exists():
                counter = 1; base = new_folder_name
    ...
    if errors:
        result.update(status="partial",
                      notes=(count_note + " | " if count_note else "") + "; ".join(errors))
    else:
        result.update(status="tagged", notes=count_note or "")
...
def print_summary(results, dry_run, output_path):
    ...
    print("=" * 60)
    print("  %s : %d" % ("Would tag" if dry_run else "Tagged", counts.get("would_tag" if dry_run else "tagged", 0)))
    if not dry_run: print("  Partial   : %d" % counts.get("partial", 0))
...
def main():
    ...
    if not args:
        try:
            ...
            print("ERROR: Could not open file picker: %s" % e); sys.exit(1)
    else:
    ...
    print("  Rows without MBID   : %d (skipped)" % (len(all_rows) - len(actionable)))
    print()
    ...
    results = []
    for idx, row in enumerate(actionable, 1):
        print("  [%d/%d]  %s" % (idx, len(actionable), row.get("folder","")))
        result = process_folder(row, DRY_RUN, SKIP_ART)
        sym = {"tagged":"✓","would_tag":"~","partial":"!","skipped":"-","error":"✗"}.get(result["status"],"?")
        if result["status"] in ("tagged","would_tag"):
            print("          %s %d file(s) → %s" % (sym, result["files_tagged"], result["new_folder"]))
        else:
