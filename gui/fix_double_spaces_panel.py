"""
Music Tools GUI -- Fix Double Spaces Panel  v0.1
================================================
GUI panel for filename_scanner/fix_double_spaces.py.
...
try:
    from filename_scanner.fix_double_spaces import run_fix_double_spaces
...
class FixDoubleSpacesPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        title_lbl = QLabel("Fix Double Spaces")
        title_lbl.setObjectName("pageTitle")
        sub_lbl = QLabel("Finds and renames files with double spaces in their filenames")
        ...
        self._table.setHorizontalHeaderLabels(
            ["Folder", "Original Filename", "New Filename", "Status"]
        )
    ...
    def _on_run(self):
        if not self._folder_edit.text().strip():
            self.log_message.emit("Fix Double Spaces: no folder selected.", "warning")
            return
    ...
    def _start_worker(self, apply: bool):
        if not _SCRIPT_AVAILABLE:
            self.log_message.emit(
                "Fix Double Spaces: script not found — check project structure.", "error"
            )
            return
        ...
        self.log_message.emit(
            f"Fix Double Spaces: {'apply' if apply else 'dry run'} — {folder}", "info"
        )
    ...
    def _on_finished(self, result: dict, was_apply: bool):
        ...
        if was_apply:
            summary = f"Renamed: {renamed}  |  Skipped: {skipped}  |  Errors: {errors}"
            ...
            summary = f"Would rename: {would}  |  Skipped: {skipped}"
            level = "info"
            self._set_state("done_dry")

        self._progress.setValue(100)
        self._summary_lbl.setText(summary)
        self.log_message.emit(f"Fix Double Spaces {'complete' if was_apply else 'dry run'} — {summary}", level)
        if report:
            self.log_message.emit(f"Report: {report}", "info")

    def _on_error(self, message: str):
        self.log_message.emit(f"Fix Double Spaces error: {message}", "error")
        self._set_state("idle")
    ...
    def _populate_table(self, rows: list[dict]):
        ...
        for row in rows:
            ...
            for col, key in enumerate(["Folder", "Original Filename", "New Filename", "Status"]):
